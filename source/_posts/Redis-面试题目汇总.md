...
27. replication buffer 大小通过什么参数设置，超过限制会有什么问题？
...
46. 使用 Redis 作为缓存时，如果采用 Write Update 方式，可能存在哪些问题？

    更新数据时，不论是先更新数据库，再更新缓存，还是先更新缓存，再更新数据库，都可能会产生不一致现象。

47. 使用 Redis 作为缓存时，一般使用的缓存策略是什么？
...
86. 如果字典正在 Rehash，此时有请求过来，Redis 会怎么处理？
...
89. Redis 的 Zset 为什么同时需要字典和跳表来实现？
...
99. Redis 性能这么高，那它是协程模型，还是多线程模型？
...
115. Redis 中字典 rehash 发生时间，以及过程？
...
121. Redis 是如何实现命令多态的？

     相同的命令，根据不同的编码选择执行不同底层数据结构的具体实现，从而实现多态。

122. Redis 是如何进行内存回收的？

     通过引用计数的方式实现，依赖 refcount 值。

123. Redis 中的对象共享池指什么，为什么没有实现对应的字符串共享池？
...
126. Redis 中的过期字典是什么，存在于什么位置？

     存在于 RedisDb 结构中，用于记录哪些设置了过期时间的键，并且保存对应的过期时间戳。

127. Redis 中的过期删除策略有哪些，如何实现的？
...
136. Redis 中保存客户端的数据结构是什么？该数据结构中有哪些关键属性？
...
147. Redis 中的 Sentinel 如何启动和初始化的？
...
152. Redis 如何存储超大量数据，如何实现分布式数据存储？

     Redis 集群是 Redis 提供的分布式数据库方案，集群通过分片（sharding） 来进行数据共享，并提供复制和故障转移功能。

153. Redis 中的节点数据结构，节点间是如何连接的？
...
158. Redis 中的节点数据库和普通服务器数据库有什么不同？
...
161. ASK 错误和 MOVED 错误的区别？

     前者发生在节点间迁移槽的时候，是一种临时措施；后者发生在槽的负责权已经从一个节点迁移到了另外一个节点。

162. Redis 集群中如何设置从节点？是如何进行故障检测和故障转移的？
...
165. 当执行 `PUBLISH <channel> <message>` 的时候，Redis 是如何进行处理的？
...
168. Redis 中的 WATCH 命令的作用，如何实现的？
...
170. Redis 中的 SORT numbers 命令的实现过程？
...
172. Redis 中的慢查询日志有什么作用，相关参数有哪些，具体实现？
