...
47. `Foo foo = Foo{10}` 与 `Foo foo{10}` 在 C++ 17 前后有何不同？
...
48. 什么是纯右值物质化？`Foo&& foo = Foo{10}` 表达了什么语意？
...
52. 已经有了左值引用，为什么还需要右值引用，右值引用有何作用？
...
63. 如何使用 rand() 函数产生均匀的 [range_min, range_max) 随机数？
...
69. C++ 支持在函数外返回局部变量的地址吗？

    C++ 不支持在函数外返回局部变量的地址，除非定义局部变量为 static 变量。

70. static_assert 和 assert 有何区别？
...
87. 列表初始化和 std::initializer_list 关系？
...
89. 为什么拷贝构造函数传递的是引用，而不是值？

    如果传入的是值，将会引起无限递归调用，另外还声明了 const，用于确保在复制构造过程中不修改被复制的对象。

90. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
93. this 指针指代的是什么，当调用成员函数的时候，this 如何传入的？
...
94. C++ 中的静态成员变量和函数与普通的成员变量和函数有何不同？
...
100. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
116. new，oprator new 和 placement new 操作符的区别？
...
118. 内存块太小导致 malloc 和 new 返回空指针，需要如何处理？
...
120. 在 C++ 中，使用 malloc 申请的内存能否通过 delete 释放？使用 new 申请的内存能否用free？
...
123. C++ 中 typename 和 class 在模板定义中区别？
...
130. 如果想要使得自动类型推演得到引用或者 const 语义？
...
137. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
154. C++ thread join 和 detach 的区别？
...
161. 容器内部如何删除一个元素，确保根据迭代器可以继续执行？
...
165. 为何 map 和 set 的插入删除效率比其他序列容器高，而且每次 insert 之后，以前保存的iterator 不会失效？

     因为存储的是结点，不需要内存拷贝和内存移动。因为插入操作只是结点指针换来换去，结点内存没有改变。而 iterator 就像指向结点的指针，内存没变，指向内存的指针也不会变。

166. unordered_map 与 map 的区别？使用场景？
...
169. emplace_back 和 push_back 区别？
...
170. 为什么要引入 `std::array` 而不是直接使用 `std::vector`？
