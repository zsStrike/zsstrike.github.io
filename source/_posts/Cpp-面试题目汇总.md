...
32. const 关键字在修饰指针时，有哪几种形式，分别有什么效果？
...
37. 一个参数可以既是 const 又是 volatile 吗？
...
50. `Foo foo = Foo{10}` 与 `Foo foo{10}` 在 C++ 17 前后有何不同？
...
51. 什么是纯右值物质化？`Foo&& foo = Foo{10}` 表达了什么语意？
...
55. 已经有了左值引用，为什么还需要右值引用，右值引用有何作用？
...
66. 如何使用 rand() 函数产生均匀的 [range_min, range_max) 随机数？
...
73. C++ 支持在函数内返回局部变量的地址吗？

    C++ 不支持在函数内返回局部变量的地址，除非定义局部变量为 static 变量。

74. static_assert 和 assert 有何区别？
...
96. 列表初始化和 std::initializer_list 关系？
...
98. 为什么拷贝构造函数传递的是引用，而不是值？

    如果传入的是值，将会引起无限递归调用，另外还声明了 const，用于确保在复制构造过程中不修改被复制的对象。

99. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
103. C++ 中的静态成员变量和函数与普通的成员变量和函数有何不同？
...
110. 模板类中能否包含虚函数，成员函数模板是否可以是虚函数？
...
113. 执行 `delete this` 后能否再次调用成员函数？
...
115. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
134. 内存不足的情况下调用 malloc 和 new，需要如何处理？
...
140. C++ 中 typename 和 class 在模板定义中区别？
...
144. std::enable_shared_from_this 有何作用？
...
155. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
178. C++ thread join 和 detach 的区别？
...
185. 容器内部如何删除一个元素，确保根据迭代器可以继续执行？
...
189. 为何 map 和 set 的插入删除效率比其他序列容器高，而且每次 insert 之后，以前保存的 iterator 不会失效？

     因为存储的是结点，不需要内存拷贝和内存移动。因为插入操作只是结点指针换来换去，结点内存没有改变。而 iterator 就像指向结点的指针，内存没变，指向内存的指针也不会变。

190. unordered_map 与 map 的区别？使用场景？
...
194. 为什么要引入 `std::array` 而不是直接使用 `std::vector`？
...
204. 为什么 static 数据成员可以是该成员所属的类类型，而非 static 数据成员只能自身类的引用或者指针？
...
209. thread 中的 join 和 detach 有何作用？
...
211. 多线程编程时，如果想要某个函数只被调用一次，应该如何处理？

     + 使用 call_once & once_flag：once_flag 需要被声明为全局，保证不同线程可以访问
     + 使用 atomic_bool：用于进行同步

212. 什么是竞争条件（race condition）和临界区（critical area）？
...
214. 当某个操作需要锁定多个 mutex 的时候，可能发生死锁，C++ 如何避免该情况的发生？

     + 可以使用提供的 lock 和 try_lock 通用锁定资源，该接口会确保不会发生死锁
     + 根据死锁产生的条件，破坏其条件即可，如按顺序加锁等

215. 为什么有了 mutex 提供的 lock 和 unlock 基本功能，还需要 lock_guard 等相关类？
...
219. 谈谈 C++ 中的并发编程，以及并发编程所涉及到类和作用？
