...
104. 如果想要使得自动类型推演得到引用或者 const 语义？
...
106. `auto i{1}` 和 `auto v = {1}`  有何区别？

     前者表示 `auto i = 1`，后者等价于 `std::initializer_list<int> v = {1}` 。

     > 因此，auto 不能用于推到数组类型

107. `decltype(auto)` 有何作用？
...
111. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
123. C++ thread join 和 detach 的区别？
...
126. emplace_back 和 push_back 区别？
...
127. 为什么要引入 `std::array` 而不是直接使用 `std::vector`？
