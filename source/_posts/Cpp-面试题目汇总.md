...
23. `Foo foo = Foo{10}` 在 C++ 17 前后有何不同？
...
24. 什么是纯右值物质化？`Foo&& foo = Foo{10}` 表达了什么语意？
...
32. C++ 全局变量、局部变量、静态全局变量、静态局部变量的区别？
...
34. const 关键字在修饰指针时，有哪几种形式，分别有什么效果？
...
43. 如何使用 rand() 函数产生均匀的 [range_min, range_max) 随机数？
...
63. 列表初始化和 std::initializer_list 关系？
...
65. 为什么拷贝构造函数传递的是引用，而不是值？

    如果传入的是值，将会引起无限递归调用。

66. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
74. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
104. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
113. C++ thread join 和 detach 的区别？
...
116. emplace_back 和 push_back 区别？
