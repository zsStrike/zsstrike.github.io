...
71. C++ 支持在函数外返回局部变量的地址吗？

    C++ 不支持在函数外返回局部变量的地址，除非定义局部变量为 static 变量。

72. static_assert 和 assert 有何区别？
...
93. 列表初始化和 std::initializer_list 关系？
...
95. 为什么拷贝构造函数传递的是引用，而不是值？

    如果传入的是值，将会引起无限递归调用，另外还声明了 const，用于确保在复制构造过程中不修改被复制的对象。

96. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
99. this 指针指代的是什么，当调用成员函数的时候，this 如何传入的？
...
100. C++ 中的静态成员变量和函数与普通的成员变量和函数有何不同？
...
107. 模板类中能否包含虚函数，成员函数模板是否可以是虚函数？
...
110. 执行 `delete this` 后能否再次调用成员函数？
...
112. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
129. new，oprator new 和 placement new 操作符的区别？
...
131. 内存块太小导致 malloc 和 new 返回空指针，需要如何处理？
...
133. 在 C++ 中，使用 malloc 申请的内存能否通过 delete 释放？使用 new 申请的内存能否用free？
...
137. C++ 中 typename 和 class 在模板定义中区别？
...
141. std::enable_shared_from_this 有何作用？
...
145. 如果想要使得自动类型推演得到引用或者 const 语义？
...
152. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
156. C++类的内部可以定义引用数据成员吗？

     可以，必须通过构造函数初始化列表初始化类内的引用。

157. 左值和右值分别是什么，之间有何区别？
...
175. C++ thread join 和 detach 的区别？
...
182. 容器内部如何删除一个元素，确保根据迭代器可以继续执行？
...
186. 为何 map 和 set 的插入删除效率比其他序列容器高，而且每次 insert 之后，以前保存的iterator 不会失效？

     因为存储的是结点，不需要内存拷贝和内存移动。因为插入操作只是结点指针换来换去，结点内存没有改变。而 iterator 就像指向结点的指针，内存没变，指向内存的指针也不会变。

187. unordered_map 与 map 的区别？使用场景？
...
190. emplace_back 和 push_back 区别？
...
191. 为什么要引入 `std::array` 而不是直接使用 `std::vector`？
...
198. 为什么 static 数据成员可以是该成员所属的类类型，而非 static 数据成员只能自身类的引用或者指针？
