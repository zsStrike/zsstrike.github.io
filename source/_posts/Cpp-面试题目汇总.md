...
34. C++ 全局变量、局部变量、静态全局变量、静态局部变量的区别？
...
36. const 关键字在修饰指针时，有哪几种形式，分别有什么效果？
...
45. 如何使用 rand() 函数产生均匀的 [range_min, range_max) 随机数？
...
67. 列表初始化和 std::initializer_list 关系？
...
69. 为什么拷贝构造函数传递的是引用，而不是值？

    如果传入的是值，将会引起无限递归调用。

70. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
78. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
101. 如果想要使得自动类型推演得到引用或者 const 语义？
...
103. `auto i{1}` 和 `auto v = {1}`  有何区别？

     前者表示 `auto i = 1`，后者等价于 `std::initializer_list<int> v = {1}` 。

     > 因此，auto 不能用于推到数组类型

104. `decltype(auto)` 有何作用？
...
108. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
119. C++ thread join 和 detach 的区别？
...
122. emplace_back 和 push_back 区别？
