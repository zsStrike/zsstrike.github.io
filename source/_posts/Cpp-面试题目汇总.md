...
93. 构造函数能否抛出异常？
    ...
    在构造函数中可以抛出异常，但是如果处理不当可能会导致内存泄漏：

    ```cpp
    ClassConstructor(const int val1, const int val2) {
    	int *a = new int[val1];
    	if (val2 == 0) {
    ...
    上述代码就可能造成内存泄露，具体的处理方式：

    + 在构造器中手动释放内存，如在 throw 语句前面增加 `delete[] a`
    + 在资源初始化之前就开始进行条件判断，这样在抛出异常的时候没有分配内存，就不存在内存泄漏的问题了
    + 提供 init 等初始化函数，避免在构造器中抛出异常
    + 使用智能指针管理成员变量

94. 析构函数能否抛出异常？

    C++并不阻止在类的析构函数中抛出异常，但这是一个非常不好的做法！因为栈展开的前提是已经有一个未处理的异常，并且栈展开会自动调用函数本地对象的析构函数，如果这时对象的析构函数时又抛出一个异常，现在就同时有两个异常出现，但C++最多只能同时处理一个异常，因此程序这时会自动调用std::terminate()函数，导致我们所谓的闪退或者崩溃。相关策略有：

    + 把可能抛出异常的代码移出析构函数
    + 析构函数中主动 catch 异常并进行相应处理
...
100. 列表初始化和 std::initializer_list 关系？
...
102. 为什么拷贝构造函数传递的是引用，而不是值？

     如果传入的是值，将会引起无限递归调用，另外还声明了 const，用于确保在复制构造过程中不修改被复制的对象。

103. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
106. this 指针指代的是什么，当调用成员函数的时候，this 如何传入的？
...
107. C++ 中的静态成员变量和函数与普通的成员变量和函数有何不同？
...
115. 模板类中能否包含虚函数，成员函数模板是否可以是虚函数？
...
118. 执行 `delete this` 后能否再次调用成员函数？
...
120. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
137. new operator，oprator new 和 placement new 操作符的区别？
...
139. 内存不足的情况下调用 malloc 和 new，需要如何处理？
...
141. 在 C++ 中，使用 malloc 申请的内存能否通过 delete 释放？使用 new 申请的内存能否用 free？
...
145. C++ 中 typename 和 class 在模板定义中区别？
...
149. std::enable_shared_from_this 有何作用？
...
160. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
164. C++ 类的内部可以定义引用数据成员吗？

     可以，必须通过构造函数初始化列表初始化类内的引用。

165. 左值和右值分别是什么，之间有何区别？
...
167. std::function 有何作用？

     封装可调用对象，提供统一的接口，可将 lambda 表达式，函数指针，函数对象赋值给 function 对象，提高程序的封装程度，可以将其当作是函数容器。

168. std::bind 和 std::placeholder 作用？

     `std::bind` 则是用来绑定函数调用的参数的，它解决的需求是我们有时候可能并不一定能够一次性获得调用某个函数的全部参数，通过这个函数，我们可以将部分调用参数提前绑定到函数身上成为一个新的对象，然后在参数齐全后，完成调用，可以用于实现柯里化。

169. C++ 如何确保在同一个文件中只能将同一个头文件包含一次？

     + ifndef-define-endif：移植性高
     + #pragma once：通用，不是标准的预处理器
     + _Pragma("once")：C99 中定义，标准，但是使用较少

170. 条件编译有何作用，与直接使用 if 语句有何区别？
...
183. C++ thread join 和 detach 的区别？
...
187. vector 中的 reserve 和 resize 的区别？
...
190. 容器内部如何删除一个元素，确保根据迭代器可以继续执行？
...
192. list 底层原理？
     ...
     list 的底层是一个双向链表，以结点为单位存放数据，结点的地址在内存中不一定连续，每次插入或删除一个元素， 就配置或释放一个元素空间。list 不支持随机存取，适合需要大量的插入和删除，而不关心随机存取的应用场景。
...
194. 为何 map 和 set 的插入删除效率比其他序列容器高，而且每次 insert 之后，以前保存的 iterator 不会失效？

     因为存储的是结点，不需要内存拷贝和内存移动。因为插入操作只是结点指针换来换去，结点内存没有改变。而 iterator 就像指向结点的指针，内存没变，指向内存的指针也不会变。

195. unordered_map 与 map 的区别？使用场景？
...
198. emplace_back 和 push_back 区别？
...
199. 为什么要引入 `std::array` 而不是直接使用 `std::vector`？
...
202. STL 是如何进行内存空间分配？
     ...
     + 第二级配置器：如果分配的区块小于 128B，则以内存池管理，第二级配置器维护了一个自由链表数组，每次需要分配内存时， 直接从相应的链表上取出一个内存节点就完成工作，效率很高。
       ...
       + 内存分配：allocate 函数内先判断要分配的内存大小，若大于 128 字节，直接调用第一级配置器，否则根据要分配的内存大小从 16 个链表中选出一个链表，取出该链表的第一个节点。若相应的链表为空，则调用 refill 函数填充该链表。 默认是取出 20 个数据块。
...
203. 内存分配器有何作用？如何自定义内存分配器 allocator？
...
209. 为什么 static 数据成员可以是该成员所属的类类型，而非 static 数据成员只能自身类的引用或者指针？
...
214. thread 中的 join 和 detach 有何作用？
...
216. 多线程编程时，如果想要某个函数只被调用一次，应该如何处理？

     + 使用 call_once & once_flag：once_flag 需要被声明为全局，保证不同线程可以访问
     + 使用 atomic_bool：用于进行同步

217. 什么是竞争条件（race condition）和临界区（critical area）？
...
219. 当某个操作需要锁定多个 mutex 的时候，可能发生死锁，C++ 如何避免该情况的发生？

     + 可以使用提供的 lock 和 try_lock 通用锁定资源，该接口会确保不会发生死锁
     + 根据死锁产生的条件，破坏其条件即可，如按顺序加锁等

220. 为什么有了 mutex 提供的 lock 和 unlock 基本功能，还需要 lock_guard 等相关类？
...
224. 谈谈 C++ 中的并发编程，以及并发编程所涉及到类和作用？
...
232. Linux 内存中的 Cache 和 Buffer 指的是什么，有何作用，如何进行释放？
...
233. Linux 中的 fflush，fsync 和 sync 的作用有何不同？
...
234. Linux 中的内存管理机制是怎样的，buddy 系统和 slab 算法分别有何作用？
     ...
     - 将空闲内存分为 m 个组, 第一组储存2^0个单位的内存块, 第二组储存2^1个单位的内存块, 第三组储存2^2个单位的储存块, 以此类推, 直到 m 组。m 典型值为 11。
...
235. 谈谈现代的 C/C++ 编译常用工具链，以及使用 GCC 编译的流程？
...
238. malloc 进行内存分配时什么时候会使用 sbrk，什么时候会使用 mmap？
...
239. 使用全局变量和 goto 语句是否对性能存在影响？

     全局变量：使用全局变量后，很有可能随后的访问覆盖了全局变量所占缓存，导致性能低下，可以使用局部变量替代。

     goto：大量使用 goto 语句会导致指令缓存和数据缓存频繁失效，导致性能低下。
