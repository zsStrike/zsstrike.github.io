...
22. C++ 全局变量、局部变量、静态全局变量、静态局部变量的区别？
...
24. const 关键字在修饰指针时，有哪几种形式，分别有什么效果？
...
32. `Foo foo = Foo{10}` 与 `Foo foo{10}` 在 C++ 17 前后有何不同？
...
33. 什么是纯右值物质化？`Foo&& foo = Foo{10}` 表达了什么语意？
...
37. 已经有了左值引用，为什么还需要右值引用，右值引用有何作用？
...
55. static_assert 和 assert 有何区别？
...
71. 列表初始化和 std::initializer_list 关系？
...
73. 为什么拷贝构造函数传递的是引用，而不是值？

    如果传入的是值，将会引起无限递归调用，另外还声明了 const，用于确保在复制构造过程中不修改被复制的对象。

74. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
77. this 指针指代的是什么，当调用成员函数的时候，this 如何传入的？
...
78. C++ 中的静态成员变量和函数与普通的成员变量和函数有何不同？
...
83. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
97. new，oprator new 和 placement new 操作符的区别？
...
101. C++ 中 typename 和 class 在模板定义中区别？
...
103. 智能指针模板类有哪些，分别有何作用？

     + auto_ptr：采用所有权转让方式实现赋值运算，避免了两次释放相同地址空间的问题，但是`auto_ptr<string> a = b`会导致 b 丢失所有权，后面再次使用可能导致异常，因此被 deprecated
...
107. 如果想要使得自动类型推演得到引用或者 const 语义？
...
114. C++ 类主要有哪几种特殊函数，隐式生成时其默认行为如何？
...
119. C++ 如何确保在同一个文件中只能将同一个头文件包含一次？
...
129. C++ thread join 和 detach 的区别？
...
132. emplace_back 和 push_back 区别？
...
133. 为什么要引入 `std::array` 而不是直接使用 `std::vector`？
