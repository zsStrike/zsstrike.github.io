...
20. 已经有了左值引用，为什么还需要右值引用，右值引用有何作用？
...
33. 如何使用 rand() 函数产生均匀的 [range_min, range_max) 随机数？
...
52. 列表初始化和 std::initializer_list 关系？
...
54. 为什么拷贝构造函数传递的是引用，而不是值？

    如果传入的是值，将会引起无限递归调用。

55. 关于为什么当类成员中含有指针类型成员且需要对其分配内存时，一定要定义拷贝构造函数？
...
63. 重载运算符时，其被当作成员函数和非成员函数在参数上有何不同？
...
94. emplace_back 和 push_back 区别？
