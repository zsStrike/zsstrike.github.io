...
+ 间隙锁：锁定一个范围，间隙锁是不互斥的，**两个不同的事务可以持有公共的间隙锁，间隙锁本质上用于阻止其他事务再该间隙内插入新记录，而自身事务允许在该间隙内插入数据**

+ Next-Key Lock：记录锁和间隙锁的组合，用左开右闭区间表示，是加锁的基本单位，但是可能会退化，普通的 select 语句是快照读，只有 `select .. in share mode` 和 `select ... for update` 才会加锁

+ 共享锁/排他锁：都只是行锁，与间隙锁无关，侧面说明了间隙锁可以共存

+ 插入意向锁：特殊的间隙锁，不同于间隙锁的是，该锁只用于并发插入操作，一个事务拥有插入意向锁，另外一个事务便不能拥有对应的间隙锁
