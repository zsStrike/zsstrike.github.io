...
14. MySQL 有哪些存储引擎（表类型），各自的优缺点分别是什么？
    ...
    + MyISAM：不支持事务、也不支持外键，支持全文索引，其优势是访问的速度快，对事务完整性没有要求或者以 SELECT、INSERT 为主的应用基本上都可以使用这个引擎创建表
