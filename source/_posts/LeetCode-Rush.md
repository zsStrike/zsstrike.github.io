...
+ 滑动窗口模板：维护一个窗口，不断滑动并且更新数据结构，相关问题如最小覆盖子串，字符串排列子串判定，找所有字母的异位词，最长无重复子串，长度最小的子数组，串联所有单词的子串（相同长度），三个无重叠子数组的最大和
...
+ 可怜的小猪：在给定 n 只小猪，m 次尝试时，每只猪提供的信息是 m + 1 次，整个的信息个数就是 pow(m + 1, n)
